...
export const ProductDetailPage: React.FC<ProductDetailPageProps> = ({
  ...
  const handleShareProduct = async () => {
    ...
    const slugOrId = product.slug || resolvedProductId || productId || productSlug;
    const shareUrl =
      isStoreContext && storeContextSlug
        ? `${window.location.origin}/p/${encodeURIComponent(
            slugOrId || ''
          )}?store=${encodeURIComponent(storeContextSlug)}`
        : `${window.location.origin}/p/${encodeURIComponent(slugOrId || '')}`;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div className="space-y-4">
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100">
              <div className="relative min-h-[360px] md:min-h-[520px] bg-white flex items-center justify-center overflow-hidden p-4">
                {isSoldOut && (
                ...
                {images.length > 0 ? (
                  <img
                    ...
                    alt={productTitle}
                    className={`max-w-full max-h-[500px] object-contain transition-transform duration-300 hover:scale-[1.02] ${isSoldOut ? 'opacity-70 grayscale' : ''}`}
                  />
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-6 gap-3">
                {images.map((image, index) => (
                  <button
                    ...
                    className={`aspect-square bg-white rounded-xl overflow-hidden border-2 transition-all hover:border-blue-500 shadow-sm ${
              ...
              <div>
                ...
                {product.slug && (
                  <div className="text-xs text-gray-500" dir="ltr">
                    {isStoreContext && storeContextSlug
                      ? `/p/${product.slug}?store=${storeContextSlug}`
                      : `/p/${product.slug}`}
                  </div>
                )}
              </div>
