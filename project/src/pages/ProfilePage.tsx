...
const getServiceMaxRevisions = (item: Pick<ProfileOrderItem, 'service_revisions_count'>) => {
  const rawValue = Number(item.service_revisions_count ?? 0);
  if (!Number.isFinite(rawValue) || rawValue < 0) return 0;
  return Math.floor(rawValue);
};

const getServiceUsedRevisions = (item: Pick<ProfileOrderItem, 'revisions_used'>) => {
  const rawValue = Number(item.revisions_used ?? 0);
  if (!Number.isFinite(rawValue) || rawValue < 0) return 0;
  return Math.floor(rawValue);
};

const getServiceRemainingRevisions = (
  item: Pick<ProfileOrderItem, 'service_revisions_count' | 'revisions_used'>
) => {
  return Math.max(getServiceMaxRevisions(item) - getServiceUsedRevisions(item), 0);
};
...
export const ProfilePage: React.FC<ProfilePageProps> = ({ onNavigate }) => {
    ...
    try {
      ...
      try {
        const { data: serviceRows, error: serviceError } = await supabase
          ...
          .select(
            'id, order_id, order_item_id, product_id, buyer_id, seller_id, buyer_requirements, seller_notes, service_status, seller_delivery_note, seller_delivery_file_url, buyer_revision_note, delivery_message, delivery_url, revision_request, delivered_at, completed_at, accepted_at, revision_requested_at, revisions_used, created_at, updated_at'
          )
          .in('order_id', orderIds);
  ...
  const handleRequestServiceRevision = async (item: ProfileOrderItem) => {
    ...
    const maxRevisions = getServiceMaxRevisions(item);
    const usedRevisions = getServiceUsedRevisions(item);

    if (maxRevisions <= 0 || usedRevisions >= maxRevisions) {
      setServiceMessage('تم استهلاك عدد التعديلات المتاحة لهذه الخدمة.');
      return;
    }
      ...
      if (rpcError) {
        const { error: updateError } = await supabase
          ...
          .update({
            ...
            revisions_used: getServiceUsedRevisions(item) + 1,
                                    ...
                                    >
                                            ...
                                            {isServiceItem && (
                                              <div className="flex flex-wrap items-center gap-2 mt-2 text-xs text-gray-500">
                                                ...
                                                item.service_revisions_count !== undefined ? (
                                                  <span>
                                                    • التعديلات: {getServiceUsedRevisions(item)} من{' '}
                                                    {getServiceMaxRevisions(item)} مستخدمة، المتبقي:{' '}
                                                    {getServiceRemainingRevisions(item)}
                                                  </span>
                                                ) : null}
                                              </div>
                                            )}
                                      ...
                                      {isServiceItem && (
                                        <div className="space-y-3">
                                          ...
                                          {normalizeServiceStatus(item.service_status) === 'delivered' && item.service_detail_id && normalizeServiceStatus(item.service_status) !== 'completed' && (
                                            <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
                                              <p className="text-sm font-bold text-gray-900">
                                                راجع التسليم، ثم اقبل الخدمة أو اطلب تعديل من التاجر.
                                              </p>
                                              <div className="rounded-lg bg-purple-50 border border-purple-100 px-3 py-2 text-xs font-semibold text-purple-700">
                                                التعديلات المستخدمة: {getServiceUsedRevisions(item)} من{' '}
                                              ...
                                              <div className="flex flex-wrap gap-2">
                                                ...
                                                <button
                                                  ...
                                                  disabled={serviceActionLoading === `${item.service_detail_id}:revision` || getServiceRemainingRevisions(item) <= 0}
                                                ...
                                                >
                                                  ...
                                                  <span>
                                                    {serviceActionLoading === `${item.service_detail_id}:revision`
                                                      ? 'جاري الإرسال...'
                                                      : getServiceRemainingRevisions(item) <= 0
                                                        ? 'لا توجد تعديلات متبقية'
                                                        : 'طلب تعديل'}
                                                  </span>
                                                </button>
                                              </div>
